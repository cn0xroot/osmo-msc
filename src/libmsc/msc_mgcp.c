...
/* Receive Assignment Complete details.
 * On Iu, we do not receive a speech_codec_chosen IE, hence the caller may pass NULL instead.
 */
int msc_mgcp_ass_complete(struct ran_conn *conn, const char *rtp_addr, uint16_t rtp_port,
			  const struct gsm0808_speech_codec *speech_codec_chosen)
{
	...
	if (!rtp_addr || strlen(rtp_addr) <= 0) {
		LOGPCONN(conn, LOGL_ERROR, "Assignment Complete: invalid remote call leg RTP address (empty)\n");
		return -EINVAL;
	}

	if (rtp_port == 0) {
		LOGPCONN(conn, LOGL_ERROR, "Assignment Complete: invalid remote call leg RTP port (0)\n");
		return -EINVAL;
	}

	osmo_strlcpy(conn->rtp.remote_addr_ran, rtp_addr, sizeof(conn->rtp.remote_addr_ran));
	conn->rtp.remote_port_ran = rtp_port;

	if (speech_codec_chosen)
		conn->rtp.codec_ran = mgcp_codec_from_sc(speech_codec_chosen);
	else {
		/* For Iu, this does not receive a speech_codec_chosen IE. Just assume this one: */
		conn->rtp.codec_ran = CODEC_AMR_8000_1;
	}
