...
static enum osmo_gsup_kind gsup_client_mux_classify(struct gsup_client_mux *gcm,
						    const struct osmo_gsup_message *gsup_msg)
{
	if (gsup_msg->kind)
		return gsup_msg->kind;

	LOGP(DLGSUP, LOGL_DEBUG, "No explicit GSUP message kind, trying to guess from message type %s\n",
	...
	case OSMO_GSUP_MSGT_MT_FORWARD_SM_REQUEST:
		return OSMO_GSUP_KIND_SMS;

	default:
		return OSMO_GSUP_KIND_SUBSCRIBER_MANAGEMENT;
	}
}
...
int gsup_client_mux_rx(struct osmo_gsup_client *gsup_client, struct msgb *msg)
{
	...
	kind = gsup_client_mux_classify(gcm, &gsup);

	if (kind <= OSMO_GSUP_KIND_UNSET || kind >= ARRAY_SIZE(gcm->rx_cb)) {
		LOGP(DLGSUP, LOGL_ERROR, "Failed to classify GSUP message target\n");
		rc = -EINVAL;
		goto msgb_free_and_return;
	}

	if (!gcm->rx_cb[kind].func) {
		LOGP(DLGSUP, LOGL_ERROR, "No receiver set up for GSUP kind %s\n", osmo_gsup_kind_name(kind));
		rc = -ENOTSUP;
		goto msgb_free_and_return;
	}

	rc = gcm->rx_cb[kind].func(gcm, gcm->rx_cb[kind].data, &gsup);
