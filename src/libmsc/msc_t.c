...
static void msc_t_send_handover_failure(struct msc_t *msc_t, enum gsm0808_cause cause)
{
	...
	struct an_apdu an_apdu = {
		...
		.msg = msc_role_nas_encode(msc_t->c.fi, &nas_enc_msg),
	};
...
static int msc_t_ho_request_decode_and_store_cb(struct osmo_fsm_inst *msc_t_fi, void *data,
						const struct nas_msg *nas_dec)
{
	struct msc_t *msc_t = msc_t_priv(msc_t_fi);

	if (nas_dec->msg_type != NAS_MSG_HANDOVER_REQUEST) {
		LOG_MSC_T(msc_t, LOGL_DEBUG, "Expected %s in incoming inter-MSC Handover message, got %s\n",
			  nas_msg_type_name(NAS_MSG_HANDOVER_REQUEST), nas_msg_type_name(nas_dec->msg_type));
		return -EINVAL;
	}

	msc_t->inter_msc.cell_id_target = nas_dec->handover_request.cell_id_target;
	msc_t->inter_msc.callref = nas_dec->handover_request.call_id;
...
{
	if (msc_role_nas_decode(msc_t->c.fi, an_apdu, msc_t_ho_request_decode_and_store_cb, NULL)) {
	 ...
	 * variable of some nas_decode implementation. The simplest is to store the entire msgb. */
...
static int msc_t_find_ran_peer_from_ho_request(struct msc_t *msc_t)
{
	...
	case MSC_NEIGHBOR_TYPE_LOCAL_RAN_PEER:
		/* That's what is expected: a local RAN peer, e.g. BSC. */
		break;
	}
						  ...
						  const struct nas_msg *nas_dec)
{
	...
	/* Copy nas_dec message to un-const so we can add the AoIP Transport Layer Address. All pointer references still
	 * remain on the same memory as nas_dec, which is fine. We're just going to encode it again right away. */
	struct nas_msg nas_enc = *nas_dec;
...
static int msc_t_send_stored_ho_request(struct msc_t *msc_t)
{
	...
	return msc_role_nas_decode(msc_t->c.fi, &msc_t->inter_msc.ho_request,
					       ...
					       const struct nas_msg *nas_dec)
{
	...
	const struct nas_handover_request_ack *r = &nas_dec->handover_request_ack;
	struct nas_msg nas_enc = *nas_dec;
	...
	nas_enc.handover_request_ack.remote_rtp = (struct osmo_sockaddr_str){};

	an_apdu.msg = msc_role_nas_encode(msc_t->c.fi, &nas_enc);
	if (!an_apdu.msg)
		return -EIO;
					       ...
					       const struct nas_msg *nas_dec)
{
	...
	default:
		LOG_MSC_T(msc_t, LOGL_ERROR, "Unexpected message during Prepare Handover procedure: %s\n",
			  nas_msg_type_name(nas_dec->msg_type));
...
static void msc_t_fsm_wait_ho_request_ack(struct osmo_fsm_inst *fi, uint32_t event, void *data)
{
	...
	case MSC_EV_FROM_RAN_UP_L2:
		...
		msc_role_nas_decode(msc_t->c.fi, an_apdu, msc_t_wait_ho_request_ack_decode_cb, an_apdu);
					       ...
					       const struct nas_msg *nas_dec)
{
	...
	case NAS_MSG_HANDOVER_COMPLETE:
		...
		mncc_reparent(msc_i->inter_msc.mncc_forwarding_to_remote_cn,
...
static void msc_t_fsm_wait_ho_complete(struct osmo_fsm_inst *fi, uint32_t event, void *data)
{
	...
	case MSC_EV_FROM_RAN_UP_L2:
		...
		msc_role_nas_decode(msc_t->c.fi, an_apdu, msc_t_wait_ho_complete_decode_cb, an_apdu);
		return;
...
void msc_t_mncc_cb(struct mncc *mncc, const union mncc_msg *mncc_msg, void *data)
{
	...
	case MNCC_RTP_CREATE:
		mncc_incoming_tx_setup_cnf(mncc, &nr);
		return;
	default:
		return;
	}
...
struct mncc *msc_t_check_call_to_handover_number(const struct gsm_mncc *msg)
{
	struct msc_t *msc_t;
	const char *handover_number;
	struct mncc_incoming_call_req req;
	struct mncc *mncc;
	...
	mncc = mncc_alloc(msc_t_vsub(msc_t),
			  msc_t->c.fi,
	...
	msc_t->inter_msc.mncc_forwarding_to_remote_cn = mncc;

	if (mncc_set_rtp_stream(mncc, msc_t->inter_msc.call_leg->rtp[RTP_TO_CN])) {
		LOG_MSC_T(msc_t, LOGL_ERROR, "Failed to set up call forwarding from remote MSC\n");
		osmo_fsm_inst_term(mncc->fi, OSMO_FSM_TERM_REGULAR, NULL);
		return NULL;
	}

	req = (struct mncc_incoming_call_req){
		.setup_req_msg = *msg,
	...
	if (mncc_incoming_start(mncc, &req)) {
		LOG_MSC_T(msc_t, LOGL_ERROR, "Failed to set up call forwarding from remote MSC\n");
		osmo_fsm_inst_term(mncc->fi, OSMO_FSM_TERM_REGULAR, NULL);
		return NULL;
	}
