...
static int msc_a_nas_dec(struct msc_a *msc_a, const struct an_apdu *an_apdu, enum msc_role from_role)
{
	int rc;
	struct msc_a_nas_dec_data d = {
		.from_role = from_role,
		.an_apdu = an_apdu,
	};
	msc_a_get(msc_a, __func__);
	rc = msc_role_nas_decode(msc_a->c.fi, an_apdu, msc_a_nas_decode_cb, &d);
...
static void msc_a_fsm_validate_l3(struct osmo_fsm_inst *fi, uint32_t event, void *data)
{
	...
	case MSC_A_EV_FROM_I_SEND_END_SIGNAL_REQUEST:
		an_apdu = data;
		msc_a_nas_dec(msc_a, an_apdu, MSC_ROLE_I);
		return;
...
static int msc_a_nas_enc_ciphering(struct msc_a *msc_a, bool umts_aka, bool retrieve_imeisv);
...
int msc_a_vlr_set_cipher_mode(void *_msc_a, bool umts_aka, bool retrieve_imeisv)
{
	...
	if (msc_a_need_classmark_for_ciphering(msc_a)) {
		int rc;
		struct nas_msg msg = {
			.msg_type = NAS_MSG_CLASSMARK_REQUEST,
		};
		rc = msc_a_nas_down(msc_a, MSC_ROLE_I, &msg);
		if (rc) {
	...
	return msc_a_nas_enc_ciphering(msc_a, umts_aka, retrieve_imeisv);
}
...
static int msc_a_nas_enc_ciphering(struct msc_a *msc_a, bool umts_aka, bool retrieve_imeisv)
{
	...
	msg = (struct nas_msg){
		.msg_type = NAS_MSG_CIPHER_MODE_COMMAND,
		.cipher_mode_command = {
			...
			.geran = {
				...
				/* for nas_a.c to store the GERAN key that is actually used */
				.chosen_key = &msc_a->geran_encr,
			},
		},
	};

	if (msc_a_nas_down(msc_a, MSC_ROLE_I, &msg)) {
	...
	if (msc_a->geran_encr.key_len)
		LOG_MSC_A(msc_a, LOGL_DEBUG, "NAS encoding chose ciphering key %s\n",
...
static void msc_a_fsm_auth_ciph(struct osmo_fsm_inst *fi, uint32_t event, void *data)
{
	...
	case MSC_A_EV_FROM_I_PROCESS_ACCESS_SIGNALLING_REQUEST:
	case MSC_A_EV_FROM_I_SEND_END_SIGNAL_REQUEST:
		msc_a_nas_dec(msc_a, data, MSC_ROLE_I);
		return;
...
static void msc_a_fsm_wait_classmark_update(struct osmo_fsm_inst *fi, uint32_t event, void *data)
{
	...
	case MSC_A_EV_CLASSMARK_UPDATE:
		...
		case MSC_A_CLASSMARK_UPDATE_THEN_CIPHERING:
			msc_a_state_chg(msc_a, MSC_A_ST_AUTH_CIPH);
			if (msc_a_nas_enc_ciphering(msc_a,
...
static void msc_a_call_leg_ran_local_addr_available(struct msc_a *msc_a)
{
	...
	msg = (struct nas_msg){
		.msg_type = NAS_MSG_ASSIGNMENT_COMMAND,
		.assignment_command = {
...
static void msc_a_fsm_communicating(struct osmo_fsm_inst *fi, uint32_t event, void *data)
{
	...
	case MSC_A_EV_FROM_T_SEND_END_SIGNAL_REQUEST:
		an_apdu = data;
		msc_a_nas_dec(msc_a, an_apdu, MSC_ROLE_T);
		return;
	...
	case MSC_A_EV_HANDOVER_REQUIRED:
		msc_ho_start(msc_a, (struct nas_handover_required*)data);
		return;
...
int msc_a_up_l3(struct msc_a *msc_a, struct msgb *msg)
{
	...
	if (is_r99 >= 0
	    && nas_dec_dtap_undup_is_duplicate(msc_a->c.fi, msc_a->n_sd_next, is_r99 ? true : false, msg)) {
...
static void msc_a_up_call_assignment_complete(struct msc_a *msc_a, const struct nas_msg *ac)
{
	struct gsm_trans *cc_trans = msc_a->cc.active_trans;
...
static void msc_a_up_call_assignment_failure(struct msc_a *msc_a, const struct nas_msg *af)
{
	struct gsm_trans *trans;
...
static void msc_a_up_sapi_n_reject(struct msc_a *msc_a, const struct nas_msg *msg)
{
...
static int msc_a_up_ho(struct msc_a *msc_a, const struct msc_a_nas_dec_data *d, uint32_t ho_fi_event)
{
	if (!msc_a->ho.fi) {
...
int msc_a_nas_dec_from_msc_i(struct msc_a *msc_a, struct msc_a_nas_dec_data *d)
{
	struct vlr_subscr *vsub = msc_a_vsub(msc_a);
	const struct nas_msg *msg = d->nas_dec;
	int rc = -99;

	switch (msg->msg_type) {

	case NAS_MSG_COMPL_L3:
	...
	case NAS_MSG_HANDOVER_REQUIRED:
...
static int msc_a_nas_dec_from_msc_t(struct msc_a *msc_a, struct msc_a_nas_dec_data *d)
{
	...
	OSMO_ASSERT(d->nas_dec);

	switch (d->nas_dec->msg_type) {
	...
	case NAS_MSG_CLASSMARK_UPDATE:
		msc_a_up_classmark_update(msc_a, d->nas_dec->classmark_update.classmark, &msc_t->classmark);
		rc = 0;
		break;
	...
	default:
		LOG_MSC_A(msc_a, LOGL_ERROR, "Message from MSC-T not implemented: %s\n",
			  nas_msg_type_name(d->nas_dec->msg_type));
		rc = -ENOTSUP;
		break;
	}
	return rc;
...
int msc_a_nas_decode_cb(struct osmo_fsm_inst *msc_a_fi, void *data, const struct nas_msg *msg)
{
	struct msc_a *msc_a = msc_a_fi_priv(msc_a_fi);
	struct msc_a_nas_dec_data *d = data;
	int rc = -99;
	...
	switch (d->from_role) {
	case MSC_ROLE_I:
		LOG_MSC_A(msc_a, LOGL_DEBUG, "NAS decode: %s\n", msg->msg_name ? : nas_msg_type_name(msg->msg_type));
		rc = msc_a_nas_dec_from_msc_i(msc_a, d);
		break;

	case MSC_ROLE_T:
		LOG_MSC_A(msc_a, LOGL_DEBUG, "NAS decode from MSC-T: %s\n",
			  msg->msg_name ? : nas_msg_type_name(msg->msg_type));
		rc = msc_a_nas_dec_from_msc_t(msc_a, d);
		break;
	...
	if (rc)
		LOG_MSC_A(msc_a, LOGL_ERROR, "NAS decode error (rc=%d) for %s from %s\n", rc, nas_msg_type_name(msg->msg_type),
			  msc_role_name(d->from_role));
	return rc;
}

/* Your typical DTAP via FORWARD_ACCESS_SIGNALLING_REQUEST */
int _msc_a_nas_down(struct msc_a *msc_a, enum msc_role to_role, const struct nas_msg *nas_msg,
...
int _msc_a_msg_down(struct msc_a *msc_a, enum msc_role to_role, uint32_t to_role_event,
		    const struct nas_msg *nas_msg,
		    const char *file, int line)
{
	struct an_apdu an_apdu = {
		.an_proto = msc_a->c.ran->an_proto,
		.msg = msc_role_nas_encode(msc_a->c.fi, nas_msg),
	};
	int rc;
	if (!an_apdu.msg)
		return -EIO;
...
int msc_a_tx_dtap_to_i(struct msc_a *msc_a, struct msgb *dtap)
{
	...
	nas_msg = (struct nas_msg){
		.msg_type = NAS_MSG_DTAP,
		.dtap = dtap,
	};
	return msc_a_nas_down(msc_a, MSC_ROLE_I, &nas_msg);
}
...
int msc_tx_common_id(struct msc_a *msc_a, enum msc_role to_role)
{
	struct vlr_subscr *vsub = msc_a_vsub(msc_a);
	struct nas_msg msg = {
		.msg_type = NAS_MSG_COMMON_ID,
		.common_id = {
			.imsi = vsub->imsi,
		},
	};

	return msc_a_nas_down(msc_a, to_role, &msg);
}
