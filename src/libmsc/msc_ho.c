...
void msc_ho_down_required_reject(struct msc_a *msc_a, enum gsm0808_cause cause)
{
	...
	struct nas_msg nas_enc_msg = {
		.msg_type = NAS_MSG_HANDOVER_REQUIRED_REJECT,
	...
	msc_a_msg_down(msc_a, MSC_ROLE_I, event, &nas_enc_msg);
}
...
void msc_ho_start(struct msc_a *msc_a, const struct nas_handover_required *ho_req)
{
	if (msc_a->ho.fi) {
...
static void msc_ho_end(struct msc_a *msc_a, bool success, enum gsm0808_cause cause)
{
	...
	if (success) {
		...
		if (msc_a->cc.mncc_forwarding_to_remote_ran) {
			mncc_release(msc_a->cc.mncc_forwarding_to_remote_ran);
		...
		if (msc_t->c.remote_to) {
			...
			mncc_reparent(msc_a->cc.mncc_forwarding_to_remote_ran,
...
static bool msc_ho_find_next_target_cell(struct msc_a *msc_a)
{
	...
	struct nas_handover_required *info = &msc_a->ho.info;
...
static void msc_ho_send_handover_request(struct msc_a *msc_a)
{
	...
	if (msc_a->cc.active_trans) {
		...
		nas_enc_msg.handover_request.geran.channel_type = &channel_type;
	}

	gsm0808_cell_id_from_cgi(&nas_enc_msg.handover_request.cell_id_serving, CELL_IDENT_WHOLE_GLOBAL, &vsub->cgi);
	nas_enc_msg.handover_request.cell_id_target = msc_a->ho.new_cell.cid;

	if (msc_a_msg_down(msc_a, MSC_ROLE_T, MSC_T_EV_FROM_A_PREPARE_HANDOVER_REQUEST, &nas_enc_msg))
...
static void msc_ho_rx_request_ack(struct msc_a *msc_a, struct msc_a_nas_dec_data *hra);
...
static void msc_ho_fsm_wait_request_ack(struct osmo_fsm_inst *fi, uint32_t event, void *data)
{
	...
	case MSC_HO_EV_RX_REQUEST_ACK:
		msc_ho_rx_request_ack(msc_a, (struct msc_a_nas_dec_data*)data);
		return;
...
void msc_ho_mncc_forward_cb(struct mncc *mncc, const union mncc_msg *mncc_msg, void *data)
{
...
static int msc_ho_start_inter_msc_call_forwarding(struct msc_a *msc_a, struct msc_t *msc_t,
						  const struct msc_a_nas_dec_data *hra)
{
	const struct osmo_gsup_message *e_info = hra->an_apdu->e_info;
	struct mncc_outgoing_call_req req;
	...
	req = (struct mncc_outgoing_call_req){
		.called_present = true,
		.called.plan = 1, /* Empirical magic number. There seem to be no enum or defines for this.
				   * The only other place setting this apparently is gsm48_decode_called(). */
		.cccap_present = true,
		.cccap = {},
	};

	if (gsm48_decode_bcd_number2(req.called.number, sizeof(req.called.number),
	...
	if (msc_a->cc.active_trans) {
		req.bearer_cap_present = true;
		req.bearer_cap = msc_a->cc.active_trans->bearer_cap;
	}

	mncc = mncc_alloc(msc_a_vsub(msc_a),
			  msc_a->ho.fi,
			  MSC_HO_EV_MNCC_FORWARDING_COMPLETE,
			  MSC_HO_EV_MNCC_FORWARDING_FAILED,
			  msc_ho_mncc_forward_cb, msc_a);

	mncc_set_rtp_stream(mncc, rtp_to_ran);
	msc_a->ho.new_cell.mncc_forwarding_to_remote_ran = mncc;
	return mncc_outgoing_start(mncc, &req);
}
...
static void msc_ho_rx_request_ack(struct msc_a *msc_a, struct msc_a_nas_dec_data *hra)
{
	...
	if (!hra->nas_dec->handover_request_ack.rr_ho_command
	    || !hra->nas_dec->handover_request_ack.rr_ho_command_len) {
		msc_ho_try_next_cell(msc_a, "Missing mandatory IE in Handover Request Acknowledge:"
				     " L3 Info (RR Handover Command)\n");
		return;
	}

	if (!hra->nas_dec->handover_request_ack.chosen_channel_present) {
		LOG_HO(msc_a, LOGL_DEBUG, "No 'Chosen Channel' IE in Handover Request Ack\n");
		msc_t->geran.chosen_channel = 0;
	} else
		msc_t->geran.chosen_channel = hra->nas_dec->handover_request_ack.chosen_channel;

	if (!hra->nas_dec->handover_request_ack.chosen_encr_alg) {
		LOG_HO(msc_a, LOGL_DEBUG, "No 'Chosen Encryption Algorithm' IE in Handover Request Ack\n");
		msc_t->geran.chosen_encr_alg = 0;
	} else {
		msc_t->geran.chosen_encr_alg = hra->nas_dec->handover_request_ack.chosen_encr_alg;
	...
	msc_t->geran.chosen_speech_version = hra->nas_dec->handover_request_ack.chosen_speech_version;
	...
	nas_enc_msg = (struct nas_msg){
		.msg_type = NAS_MSG_HANDOVER_COMMAND,
		.handover_command = {
			.rr_ho_command = hra->nas_dec->handover_request_ack.rr_ho_command,
			.rr_ho_command_len = hra->nas_dec->handover_request_ack.rr_ho_command_len,
		},
	};
	...
	msc_a->ho.new_cell.ran_remote_rtp = hra->nas_dec->handover_request_ack.remote_rtp;
	...
	msc_a->ho.new_cell.codec_present = hra->nas_dec->handover_request_ack.codec_present;
	msc_a->ho.new_cell.codec = hra->nas_dec->handover_request_ack.codec;
	if (hra->nas_dec->handover_request_ack.codec_present) {
...
static void msc_ho_rtp_switch_to_new_cell(struct msc_a *msc_a)
{
	...
	if (msc_a->cc.mncc_forwarding_to_remote_ran) {
		...
		mncc_detach_rtp_stream(msc_a->cc.mncc_forwarding_to_remote_ran);
	}
...
static void msc_ho_rtp_rollback_to_old_cell(struct msc_a *msc_a)
{
	...
	if (msc_a->ho.new_cell.mncc_forwarding_to_remote_ran)
		mncc_detach_rtp_stream(msc_a->ho.new_cell.mncc_forwarding_to_remote_ran);

	/* If before this handover, there was a call forwarding to a remote MSC in place, this now goes back into
	 * responsibility. */
	if (msc_a->cc.mncc_forwarding_to_remote_ran)
		mncc_set_rtp_stream(msc_a->cc.mncc_forwarding_to_remote_ran, rtp_to_ran);
...
static void msc_ho_send_handover_succeeded(struct msc_a *msc_a)
{
	...
	if (msc_a_msg_down(msc_a, MSC_ROLE_I, MSC_I_EV_FROM_A_FORWARD_ACCESS_SIGNALLING_REQUEST, &nas_enc_msg))
