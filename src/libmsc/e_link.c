...
void msc_a_i_t_gsup_init(struct gsm_network *net)
{
	...
	msc_a_remote_fsm_init();
	msc_i_remote_fsm_init();
	msc_t_remote_fsm_init();

	net->gcm->rx_cb[OSMO_GSUP_KIND_INTER_MSC] = (struct gsup_client_mux_rx_cb){
...
/* Point a struct msgb's data members directly at the data buffer of gsup_msg->an_apdu. This *is* a hack, and the msgb
...
struct msgb *gsup_msg_to_fake_msgb(const struct osmo_gsup_message *gsup_msg)
{
	static struct msgb pdu;

	/* de-const so msgb can point at it */
	uint8_t *pdu_data = (uint8_t*)gsup_msg->an_apdu.data;
	uint8_t pdu_len = gsup_msg->an_apdu.data_len;

	/* A "fake" msgb to point at the raw data received from GSUP without copying. */
	pdu = (struct msgb){
		.data = pdu_data,
		.head = pdu_data,
		.tail = pdu_data,
	...
	if (pdu_len) {
		pdu.len = gsup_msg->an_apdu.data_len,
		pdu.tail = pdu.data + pdu.len;
	}
	return &pdu;
}
