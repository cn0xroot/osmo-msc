...
static void msub_check_for_release(struct osmo_fsm_inst *fi)
{
	...
	if (role_present[MSC_ROLE_A]
		...
		|| msub_msc_a(msub)->c.ran->type == OSMO_RAT_EUTRAN_SGS))
		return;
...
struct msub *msub_alloc(struct gsm_network *net)
{
	...
	msub = talloc_zero(msub_fi, struct msub);
	OSMO_ASSERT(msub);
	msub_fi->priv = msub;
	*msub = (struct msub){
		.net = net,
...
struct msgb *msc_role_nas_encode(struct osmo_fsm_inst *fi, const struct nas_msg *nas_msg)
{
	struct msc_role_common *c = fi->priv;
	struct msgb *msg;
	if (!c->ran->nas_encode) {
		LOGPFSML(fi, LOGL_ERROR, "Cannot encode %s: no NAS encoding function defined for RAN type %s\n",
			 nas_msg_type_name(nas_msg->msg_type), osmo_rat_type_name(c->ran->type));
		return NULL;
	}
	msg = c->ran->nas_encode(fi, nas_msg);
	if (!msg)
		LOGPFSML(fi, LOGL_ERROR, "Failed to encode %s\n", nas_msg_type_name(nas_msg->msg_type));
	return msg;
}

int msc_role_nas_decode(struct osmo_fsm_inst *fi, const struct an_apdu *an_apdu,
			nas_decode_cb_t decode_cb, void *decode_cb_data)
{
	struct nas_dec nas_dec;
	struct msc_role_common *c = fi->priv;
	if (!an_apdu) {
	...
	nas_dec = (struct nas_dec) {
		.caller_fi = fi,
		.caller_data = decode_cb_data,
		.decode_cb = decode_cb,
	};
	if (!c->ran->nas_dec_l2) {
		LOGPFSML(fi, LOGL_ERROR, "No nas_dec_l2() defined for RAN type %s\n",
			 osmo_rat_type_name(c->ran->type));
		return -ENOTSUP;
	}
	return c->ran->nas_dec_l2(&nas_dec, an_apdu->msg);
}
