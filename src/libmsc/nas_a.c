...
#define LOG_NAS_A_DEC(NAS_DEC, level, fmt, args...) \
	LOG_NAS_DEC(NAS_DEC, DBSSAP, level, "BSSMAP: " fmt, ## args)

/* Assumes presence of struct nas_dec *nas_dec and nas_dec_msg.msg_name (set) in the local scope. */
#define LOG_NAS_A_DEC_MSG(level, fmt, args...) \
	LOG_NAS_DEC(nas_dec, DBSSAP, level, "%s: " fmt, nas_dec_msg.msg_name, ## args)

#define LOG_NAS_A_ENC(FI, level, fmt, args...) \
	LOG_NAS_ENC(FI, DBSSAP, level, "BSSMAP: " fmt, ## args)

static int nas_a_decode_l3_compl(struct nas_dec *nas_dec, struct msgb *msg, struct tlv_parsed *tp)
{
	...
	if (!ie_cell_id) {
		LOG_NAS_A_DEC_MSG(LOGL_ERROR, "Mandatory CELL IDENTIFIER not present, discarding message\n");
		return -EINVAL;
	}
	if (!ie_l3_info) {
		LOG_NAS_A_DEC_MSG(LOGL_ERROR, "Mandatory LAYER 3 INFORMATION not present, discarding message\n");
		return -EINVAL;
	}
	...
	if (rc < 0) {
		LOG_NAS_A_DEC_MSG(LOGL_ERROR, "Decoding CELL IDENTIFIER gave rc=%d\n", rc);
		return -EINVAL;
	}
	if (cil.id_list_len != 1) {
		LOG_NAS_A_DEC_MSG(LOGL_ERROR, "Unable to parse element CELL IDENTIFIER, discarding message\n");
		return -EINVAL;
	}
	...
	case CELL_IDENT_BSS:
	default:
		LOG_NAS_A_DEC_MSG(LOGL_ERROR, "CELL IDENTIFIER does not specify a LAC, discarding message: %s\n",
	...
	if (msgb_l3len(msg) < sizeof(struct gsm48_hdr)) {
		LOG_NAS_A_DEC_MSG(LOGL_ERROR, "too short L3 info (%d), discarding message\n", msgb_l3len(msg));
		return -ENODATA;
	}

	return nas_decoded(nas_dec, &nas_dec_msg);
}

static int nas_a_decode_clear_request(struct nas_dec *nas_dec, struct msgb *msg, struct tlv_parsed *tp)
{
	...
	if (!ie_cause) {
		LOG_NAS_A_DEC_MSG(LOGL_ERROR, "Cause code is missing, using GSM0808_CAUSE_EQUIPMENT_FAILURE\n");
		nas_dec_msg.clear_request.bssap_cause = GSM0808_CAUSE_EQUIPMENT_FAILURE;
	} else {
		nas_dec_msg.clear_request.bssap_cause = ie_cause->val[0];
	}

	return nas_decoded(nas_dec, &nas_dec_msg);
}

static int nas_a_decode_clear_complete(struct nas_dec *nas_dec, struct msgb *msg, struct tlv_parsed *tp)
{
	struct nas_msg nas_dec_msg = {
...
static int nas_a_decode_classmark_update(struct nas_dec *nas_dec, struct msgb *msg, struct tlv_parsed *tp)
{
	...
	struct nas_msg nas_dec_msg = {
		.msg_type = NAS_MSG_CLASSMARK_UPDATE,
	...
	if (!ie_cm2) {
		LOG_NAS_A_DEC_MSG(LOGL_ERROR, "mandatory Classmark Information Type 2 not present, discarding message\n");
		return -EINVAL;
	}
...
static int nas_a_decode_cipher_mode_complete(struct nas_dec *nas_dec, struct msgb *msg, struct tlv_parsed *tp)
{
	...
	if (ie_chosen_encr_alg) {
		...
		if (ie_val < 1 || ie_val > 8)
			LOG_NAS_A_DEC_MSG(LOGL_ERROR, "Unsupported value for 3.2.2.44 Chosen Encryption Algorithm: %u\n",
					 ie_val);
		else
			nas_dec_msg.cipher_mode_complete.alg_id = ie_chosen_encr_alg->val[0];
	}

	rc = nas_decoded(nas_dec, &nas_dec_msg);
	...
	if (ie_l3_msg) {
		...
		nas_decoded(nas_dec, &nas_dec_msg);
	}

	return rc;
}

static int nas_a_decode_cipher_mode_reject(struct nas_dec *nas_dec, struct msgb *msg, struct tlv_parsed *tp)
{
	int rc;
	struct nas_msg nas_dec_msg = {
	...
	if (rc < 0) {
		LOG_NAS_A_DEC_MSG(LOGL_ERROR, "failed to extract Cause\n");
		nas_dec_msg.cipher_mode_reject.bssap_cause = GSM0808_CAUSE_EQUIPMENT_FAILURE;
	} else {
		nas_dec_msg.cipher_mode_reject.bssap_cause = (enum gsm0808_cause)rc;
	}

	return nas_decoded(nas_dec, &nas_dec_msg);
}

enum mgcp_codecs nas_a_mgcp_codec_from_sc(const struct gsm0808_speech_codec *sc)
{
	switch (sc->type) {
...
static int nas_a_decode_assignment_complete(struct nas_dec *nas_dec, struct msgb *msg, struct tlv_parsed *tp)
{
	...
	if (ie_aoip_transp_addr) {
		...
		if (rc < 0) {
			LOG_NAS_A_DEC_MSG(LOGL_ERROR, "Unable to decode AoIP Transport Layer Address\n");
			return -EINVAL;
		}
		...
		if (rtp_addr.ss_family != AF_INET) {
			LOG_NAS_A_DEC_MSG(LOGL_ERROR, "Assignment Complete: IE AoIP Transport Address:"
				 " unsupported addressing scheme (only IPV4 supported)\n");
			return -EINVAL;
		}

		if (osmo_sockaddr_str_from_sockaddr_in(&nas_dec_msg.assignment_complete.remote_rtp, rtp_addr_in)) {
			LOG_NAS_A_DEC_MSG(LOGL_ERROR, "Assignment Complete: unable to decode remote RTP IP address\n");
			return -EINVAL;
		}
	}
	...
	if (ie_speech_codec) {
		...
		if (rc < 0) {
			LOG_NAS_A_DEC_MSG(LOGL_ERROR, "Assignment Complete: unable to decode IE Speech Codec (Chosen)"
					  " (rc=%d).\n", rc);
			return -EINVAL;
		}
		nas_dec_msg.assignment_complete.codec_present = true;
		nas_dec_msg.assignment_complete.codec = nas_a_mgcp_codec_from_sc(&sc);
	}

	return nas_decoded(nas_dec, &nas_dec_msg);
}

static int nas_a_decode_assignment_failure(struct nas_dec *nas_dec, struct msgb *msg, struct tlv_parsed *tp)
{
	struct tlv_p_entry *ie_cause = TLVP_GET(tp, GSM0808_IE_CAUSE);
	...
	if (ie_cause)
		nas_dec_msg.assignment_failure.bssap_cause = ie_cause->val[0];
	if (ie_rr_cause)
		nas_dec_msg.assignment_failure.rr_cause = ie_rr_cause->val[0];

	if (ie_speech_codec_list
	    && gsm0808_dec_speech_codec_list(&scl, ie_speech_codec_list->val, ie_speech_codec_list->len) == 0)
		nas_dec_msg.assignment_failure.scl_bss_supported = &scl;

	return nas_decoded(nas_dec, &nas_dec_msg);
}

static int nas_a_decode_sapi_n_reject(struct nas_dec *nas_dec, struct msgb *msg, struct tlv_parsed *tp)
{
	struct tlv_p_entry *ie_cause = TLVP_GET(tp, GSM0808_IE_CAUSE);
	struct tlv_p_entry *ie_dlci = TLVP_GET(tp, GSM0808_IE_DLCI);
	struct nas_msg nas_dec_msg = {
		.msg_type = NAS_MSG_SAPI_N_REJECT,
	...
	if (!ie_cause) {
		LOG_NAS_A_DEC_MSG(LOGL_ERROR, "SAPI-N Reject: cause code IE is missing, discarding message\n");
		return -EINVAL;
	}
	nas_dec_msg.sapi_n_reject.bssap_cause = ie_cause->val[0];

	if (!ie_dlci) {
		LOG_NAS_A_DEC_MSG(LOGL_ERROR, "SAPI-N Reject: DLCI IE is missing, discarding message\n");
		return -EINVAL;
	}
	nas_dec_msg.sapi_n_reject.dlci = ie_dlci->val[0];

	return nas_decoded(nas_dec, &nas_dec_msg);
}

static int nas_a_decode_lcls_notification(struct nas_dec *nas_dec, const struct msgb *msg, const struct tlv_parsed *tp)
{
	...
	if ((!ie_lcls_bss_status && !ie_lcls_break_req)
		...
		LOG_NAS_A_DEC(nas_dec, LOGL_ERROR, "Ignoring broken LCLS Notification message\n");
		return -EINVAL;
	}

	if (ie_lcls_bss_status) {
		nas_dec_msg = (struct nas_msg){
			.msg_type = NAS_MSG_LCLS_STATUS,
	...
	if (ie_lcls_break_req) {
		nas_dec_msg = (struct nas_msg){
			.msg_type = NAS_MSG_LCLS_BREAK_REQ,
...
static int nas_a_decode_handover_required(struct nas_dec *nas_dec, const struct msgb *msg, const struct tlv_parsed *tp)
{
	...
	struct nas_msg nas_dec_msg = {
		.msg_type = NAS_MSG_HANDOVER_REQUIRED,
		.msg_name = "BSSMAP Handover Required",
	};
	/* On decoding failures, dispatch an invalid NAS_MSG_HANDOVER_REQUIRED so msc_a can pass down a
	 * BSS_MAP_MSG_HANDOVER_REQUIRED_REJECT	message. */

	if (ie_cause)
		nas_dec_msg.handover_required.cause = ie_cause->val[0];
	else
		LOG_NAS_A_DEC_MSG(LOGL_ERROR, "Cause IE missing\n");

	if (!ie_cil
	    || gsm0808_dec_cell_id_list2(&nas_dec_msg.handover_required.cil, ie_cil->val, ie_cil->len) <= 0) {
		LOG_NAS_A_DEC_MSG(LOGL_ERROR, "No or invalid Cell Identifier List IE\n");
		nas_dec_msg.handover_required.cil = (struct gsm0808_cell_id_list2){};
	}
...
static int nas_a_decode_handover_request(struct nas_dec *nas_dec, const struct msgb *msg, const struct tlv_parsed *tp)
{
	struct osmo_gsm48_classmark classmark = {};
	struct nas_msg nas_dec_msg = {
		.msg_type = NAS_MSG_HANDOVER_REQUEST,
		.msg_name = "BSSMAP Handover Request",
		.handover_request = {
			.classmark = &classmark,
		},
	};
	struct nas_handover_request *r = &nas_dec_msg.handover_request;
	...
	if (!ie_channel_type) {
		LOG_NAS_A_DEC_MSG(LOGL_ERROR, "Mandatory IE missing: Channel Type\n");
		return -EINVAL;
	}
	if (gsm0808_dec_channel_type(&channel_type, ie_channel_type->val, ie_channel_type->len) <= 0) {
		LOG_NAS_A_DEC_MSG(LOGL_ERROR, "Failed to decode Channel Type IE\n");
		return -EINVAL;
	}
	...
	if (!ie_classmark1 && !ie_classmark2) {
		LOG_NAS_A_DEC_MSG(LOGL_ERROR, "Mandatory IE missing: either Classmark Information 1"
	...
	if (ie_classmark1) {
		if (ie_classmark1->len != sizeof(classmark.classmark1)) {
			LOG_NAS_A_DEC_MSG(LOGL_ERROR, "Invalid size for Classmark 1: %u, expected %zu\n",
	...
	if (!ie_cell_id_serving) {
		LOG_NAS_A_DEC_MSG(LOGL_ERROR, "Mandatory IE missing: Cell Identifier (Serving)\n");
		return -EINVAL;
	}
	if (gsm0808_dec_cell_id(&r->cell_id_serving, ie_cell_id_serving->val,
				ie_cell_id_serving->len) <= 0) {
		LOG_NAS_A_DEC_MSG(LOGL_ERROR, "Failed to decode Cell Identifier (Serving) IE\n");
		return -EINVAL;
	}

	if (!ie_cell_id_target) {
		LOG_NAS_A_DEC_MSG(LOGL_ERROR, "Mandatory IE missing: Cell Identifier (Target)\n");
		return -EINVAL;
	}
	if (gsm0808_dec_cell_id(&r->cell_id_target, ie_cell_id_target->val,
				ie_cell_id_target->len) <= 0) {
		LOG_NAS_A_DEC_MSG(LOGL_ERROR, "Failed to decode Cell Identifier (Target) IE\n");
		return -EINVAL;
	}
	...
	if (ie_aoip_transp_addr) {
		do {
			...
			if (gsm0808_dec_aoip_trasp_addr(&rtp_addr, ie_aoip_transp_addr->val, ie_aoip_transp_addr->len) < 0) {
				LOG_NAS_A_DEC_MSG(LOGL_ERROR, "unable to decode AoIP transport address\n");
				break;
			}
			if (rtp_addr.ss_family != AF_INET) {
				LOG_NAS_A_DEC_MSG(LOGL_ERROR, "IE AoIP Transport Address:"
						 " unsupported addressing scheme (only IPV4 supported)\n");
				break;
			}
			if (osmo_sockaddr_str_from_sockaddr_in(&rtp_ran_local, (struct sockaddr_in*)&rtp_addr)) {
				LOG_NAS_A_DEC_MSG(LOGL_ERROR, "unable to decode remote RTP IP address\n");
				break;
			}
...
static int nas_a_decode_handover_request_ack(struct nas_dec *nas_dec, const struct msgb *msg, const struct tlv_parsed *tp)
{
	struct nas_msg nas_dec_msg = {
		.msg_type = NAS_MSG_HANDOVER_REQUEST_ACK,
	...
	/* On missing mandatory IEs, dispatch an invalid NAS_MSG_HANDOVER_REQUEST_ACK so msc_a can act on the failure. */

	if (ie_l3_info) {
		nas_dec_msg.handover_request_ack.rr_ho_command = ie_l3_info->val;
		nas_dec_msg.handover_request_ack.rr_ho_command_len = ie_l3_info->len;
	}

	if (ie_chosen_channel) {
		nas_dec_msg.handover_request_ack.chosen_channel_present = true;
		nas_dec_msg.handover_request_ack.chosen_channel = *ie_chosen_channel->val;
	}

	if (ie_chosen_encr_alg) {
		nas_dec_msg.handover_request_ack.chosen_encr_alg = *ie_chosen_encr_alg->val;
		if (nas_dec_msg.handover_request_ack.chosen_encr_alg < 1
		    || nas_dec_msg.handover_request_ack.chosen_encr_alg > 8) {
			LOG_NAS_A_DEC_MSG(LOGL_ERROR, "invalid Chosen Encryption Algorithm: %u\n",
					 nas_dec_msg.handover_request_ack.chosen_encr_alg);
		}
	}

	if (ie_chosen_speech_version) {
		struct gsm0808_speech_codec sc;
		nas_dec_msg.handover_request_ack.chosen_speech_version = ie_chosen_speech_version->val[0];

		/* the codec may be extrapolated from this Speech Version or below from Speech Codec */
		gsm0808_speech_codec_from_chan_type(&sc, nas_dec_msg.handover_request_ack.chosen_speech_version);
		nas_dec_msg.handover_request_ack.codec_present = true;
		nas_dec_msg.handover_request_ack.codec = nas_a_mgcp_codec_from_sc(&sc);
	}
	...
	if (ie_aoip_transp_addr) {
		do {
			...
			if (osmo_sockaddr_str_from_sockaddr_in(&nas_dec_msg.handover_request_ack.remote_rtp,
							  (struct sockaddr_in*)&rtp_addr)) {
				LOG_NAS_A_DEC_MSG(LOGL_ERROR, "unable to decode remote RTP IP address\n");
				nas_dec_msg.handover_request_ack.remote_rtp = (struct osmo_sockaddr_str){};
				break;
			}
		} while(0);
	}
	...
	if (ie_speech_codec) {
		...
		if (gsm0808_dec_speech_codec(&sc, ie_speech_codec->val, ie_speech_codec->len) < 0)
			LOG_NAS_A_DEC_MSG(LOGL_ERROR, "unable to decode IE Speech Codec (Chosen)\n");
		else {
...
static int nas_a_decode_handover_detect(struct nas_dec *nas_dec, const struct msgb *msg, const struct tlv_parsed *tp)
{
	struct nas_msg nas_dec_msg = {
		.msg_type = NAS_MSG_HANDOVER_DETECT,
...
static int nas_a_decode_handover_succeeded(struct nas_dec *nas_dec, const struct msgb *msg, const struct tlv_parsed *tp)
{
	struct nas_msg nas_dec_msg = {
		.msg_type = NAS_MSG_HANDOVER_SUCCEEDED,
...
static int nas_a_decode_handover_complete(struct nas_dec *nas_dec, const struct msgb *msg, const struct tlv_parsed *tp)
{
	struct nas_msg nas_dec_msg = {
		.msg_type = NAS_MSG_HANDOVER_COMPLETE,
...
static int nas_a_decode_handover_failure(struct nas_dec *nas_dec, const struct msgb *msg, const struct tlv_parsed *tp)
{
	struct nas_msg nas_dec_msg = {
		.msg_type = NAS_MSG_HANDOVER_FAILURE,
...
static int nas_a_decode_bssmap(struct nas_dec *nas_dec, struct msgb *bssmap)
{
	...
	if (msgb_l3len(bssmap) < 1) {
		LOG_NAS_A_DEC(nas_dec, LOGL_ERROR, "No data received, discarding message\n");
		return -1;
	}

	if (msgb_l3len(bssmap) < h->length) {
		LOG_NAS_A_DEC(nas_dec, LOGL_ERROR, "BSSMAP data truncated, discarding message\n");
		return -1;
	}
	...
	if (rc < 0) {
		LOG_NAS_A_DEC(nas_dec, LOGL_ERROR, "Failed parsing TLV, discarding message\n");
		return -EINVAL;
	}

	LOG_NAS_A_DEC(nas_dec, LOGL_DEBUG, "Rx BSSMAP DT1 %s\n", gsm0808_bssmap_name(msg_type));

	switch (msg_type) {
	case BSS_MAP_MSG_COMPLETE_LAYER_3:
		return nas_a_decode_l3_compl(nas_dec, bssmap, tp);
	case BSS_MAP_MSG_CLEAR_RQST:
		return nas_a_decode_clear_request(nas_dec, bssmap, tp);
	case BSS_MAP_MSG_CLEAR_COMPLETE:
		return nas_a_decode_clear_complete(nas_dec, bssmap, tp);
	case BSS_MAP_MSG_CLASSMARK_UPDATE:
		return nas_a_decode_classmark_update(nas_dec, bssmap, tp);
	case BSS_MAP_MSG_CIPHER_MODE_COMPLETE:
		return nas_a_decode_cipher_mode_complete(nas_dec, bssmap, tp);
	case BSS_MAP_MSG_CIPHER_MODE_REJECT:
		return nas_a_decode_cipher_mode_reject(nas_dec, bssmap, tp);
	case BSS_MAP_MSG_ASSIGMENT_COMPLETE:
		rc = nas_a_decode_assignment_complete(nas_dec, bssmap, tp);
		if (rc < 0) {
			struct nas_msg nas_dec_msg = {
				.msg_type = NAS_MSG_ASSIGNMENT_FAILURE,
	...
	case BSS_MAP_MSG_ASSIGMENT_FAILURE:
		return nas_a_decode_assignment_failure(nas_dec, bssmap, tp);
	case BSS_MAP_MSG_SAPI_N_REJECT:
		return nas_a_decode_sapi_n_reject(nas_dec, bssmap, tp);
	case BSS_MAP_MSG_LCLS_NOTIFICATION:
		return nas_a_decode_lcls_notification(nas_dec, bssmap, tp);

	/* From current RAN peer, the Handover origin: */
	case BSS_MAP_MSG_HANDOVER_REQUIRED:
		return nas_a_decode_handover_required(nas_dec, bssmap, tp);

	/* From current MSC to remote handover target MSC */
	case BSS_MAP_MSG_HANDOVER_RQST:
		return nas_a_decode_handover_request(nas_dec, bssmap, tp);

	/* From potential new RAN peer, the Handover target: */
	case BSS_MAP_MSG_HANDOVER_RQST_ACKNOWLEDGE:
		return nas_a_decode_handover_request_ack(nas_dec, bssmap, tp);
	case BSS_MAP_MSG_HANDOVER_DETECT:
		return nas_a_decode_handover_detect(nas_dec, bssmap, tp);
	case BSS_MAP_MSG_HANDOVER_SUCCEEDED:
		return nas_a_decode_handover_succeeded(nas_dec, bssmap, tp);
	case BSS_MAP_MSG_HANDOVER_COMPLETE:
		return nas_a_decode_handover_complete(nas_dec, bssmap, tp);

	/* From any Handover peer: */
	case BSS_MAP_MSG_HANDOVER_FAILURE:
		return nas_a_decode_handover_failure(nas_dec, bssmap, tp);

	default:
		LOG_NAS_A_DEC(nas_dec, LOGL_ERROR, "Unimplemented msg type: %s\n", gsm0808_bssmap_name(msg_type));
		return -EINVAL;
	}
...
static int nas_a_decode_l3(struct nas_dec *nas_dec, struct msgb *l3)
{
...
int nas_a_decode_l2(struct nas_dec *nas_dec, struct msgb *bssap)
{
	uint8_t bssap_type;
	OSMO_ASSERT(bssap);

	if (!msgb_l2(bssap) || !msgb_l2len(bssap)) {
		LOG_NAS_A_DEC(nas_dec, LOGL_ERROR, "Cannot decode L2, msg->l2h is unset / empty: %s\n",
			      msgb_hexdump(bssap));
		return -EINVAL;
	}

	if (msgb_l2len(bssap) < sizeof(struct bssmap_header)) {
		LOG_NAS_A_DEC(nas_dec, LOGL_ERROR, "The header is too short -- discarding message\n");
		return -EINVAL;
	}
	...
	case BSSAP_MSG_BSS_MANAGEMENT:
		return nas_a_decode_bssmap(nas_dec, bssap);
	case BSSAP_MSG_DTAP:
		return nas_a_decode_l3(nas_dec, bssap);
	default:
		LOG_NAS_A_DEC(nas_dec, LOGL_ERROR, "Unimplemented BSSAP msg type: %s\n", gsm0808_bssap_name(bssap_type));
		return -EINVAL;
	}
}

static struct msgb *nas_a_wrap_dtap(struct msgb *dtap)
{
	struct msgb *an_apdu;
	dtap->l3h = dtap->data;
...
static int nas_a_channel_type_to_speech_codec_list(struct gsm0808_speech_codec_list *scl, const struct gsm0808_channel_type *ct)
{
	unsigned int i;
	int rc;
...
static struct msgb *nas_a_make_assignment_command(struct osmo_fsm_inst *log_fi,
						  const struct nas_assignment_command *ac)
{
	struct gsm0808_speech_codec_list scl;
	...
	if (!ac->channel_type) {
		LOG_NAS_A_ENC(log_fi, LOGL_ERROR, "Assignment Command: missing Channel Type\n");
		return NULL;
	}

	if (ac->channel_type->ch_indctr == GSM0808_CHAN_SPEECH) {
		rc = nas_a_channel_type_to_speech_codec_list(&scl, ac->channel_type);
		if (rc < 0) {
			LOG_NAS_A_ENC(log_fi, LOGL_ERROR, "Assignment Command: Cannot translate Channel Type to Speech Codec List\n");
			return NULL;
		}
		use_scl = &scl;
		...
		if (osmo_sockaddr_str_is_set(ac->cn_rtp)) {
			...
			if (rtp_addr_in.sin_addr.s_addr == INADDR_NONE) {
				LOG_NAS_A_ENC(log_fi, LOGL_ERROR, "Assignment Command: Invalid RTP-Address\n");
				return NULL;
			}
			if (rtp_addr_in.sin_port == 0) {
				LOG_NAS_A_ENC(log_fi, LOGL_ERROR, "Assignment Command: Invalid RTP-Port\n");
				return NULL;
			}
				       ...
				       uint8_t a5_encryption_mask, const struct osmo_gsm48_classmark *cm)
{
	...
	for (i = 0; i < 8; i++) {
		...
		if (a5_n_to_gsm0808_chosen_enc_alg(&ei->perm_algo[j], i)) {
			LOG_NAS_A_ENC(fi, LOGL_ERROR, "Not supported: A5/%d algorithm\n", i);
			return -1;
		}
		j++;
		ei->perm_algo_len = j;
...
/* For nas_a_make_cipher_mode_command(), for
...
static struct msgb *nas_a_make_cipher_mode_command(struct osmo_fsm_inst *fi, const struct nas_cipher_mode_command *cm)
{
	...
	if (ei.perm_algo_len == 0) {
		LOG_NAS_A_ENC(fi, LOGL_ERROR, "cannot start ciphering, no intersection between MSC-configured"
	...
	if (cm->geran.chosen_key) {
		if (ei.key_len > sizeof(cm->geran.chosen_key->key)) {
			LOG_NAS_A_ENC(fi, LOGL_ERROR, "Chosen key is larger than I can store\n");
			return NULL;
		}
	...
	LOG_NAS_A_ENC(fi, LOGL_DEBUG, "Tx BSSMAP CIPHER MODE COMMAND to BSC, %u ciphers (%s) key %s\n",
...
struct msgb *nas_a_make_handover_request(struct osmo_fsm_inst *log_fi, const struct nas_handover_request *n)
{
	...
	if (!n->geran.channel_type) {
		LOG_NAS_A_ENC(log_fi, LOGL_ERROR, "Channel Type required for encoding Handover Request in BSSAP\n");
		return NULL;
	}
	...
	if (n->geran.chosen_encryption && n->geran.chosen_encryption->key_len) {
		if (n->geran.chosen_encryption->key_len > sizeof(r.encryption_information.key)) {
			LOG_NAS_A_ENC(log_fi, LOGL_ERROR, "Handover Request: invalid chosen encryption key size %u\n",
	...
	if (osmo_sockaddr_str_is_set(n->rtp_ran_local)) {
		if (osmo_sockaddr_str_to_sockaddr(n->rtp_ran_local, &ss)) {
			LOG_NAS_A_ENC(log_fi, LOGL_ERROR,
...
static struct msgb *nas_a_make_handover_request_ack(struct osmo_fsm_inst *caller_fi, const struct nas_handover_request_ack *r)
{
	struct sockaddr_storage ss;
...
struct msgb *nas_a_make_handover_command(struct osmo_fsm_inst *log_fi, const struct nas_handover_command *n)
{
...
struct msgb *nas_a_make_handover_failure(struct osmo_fsm_inst *log_fi, const struct nas_msg *msg)
{
...
static struct msgb *_nas_a_encode(struct osmo_fsm_inst *caller_fi, const struct nas_msg *nas_enc_msg)
{

	LOG_NAS_A_ENC(caller_fi, LOGL_DEBUG, "%s\n", nas_msg_type_name(nas_enc_msg->msg_type));

	switch (nas_enc_msg->msg_type) {

	case NAS_MSG_DTAP:
		return nas_a_wrap_dtap(nas_enc_msg->dtap);

	case NAS_MSG_CLASSMARK_REQUEST:
		return gsm0808_create_classmark_request();

	case NAS_MSG_CLEAR_COMMAND:
		return gsm0808_create_clear_command2(nas_enc_msg->clear_command.gsm0808_cause,
						     nas_enc_msg->clear_command.csfb_ind);

	case NAS_MSG_ASSIGNMENT_COMMAND:
		return nas_a_make_assignment_command(caller_fi, &nas_enc_msg->assignment_command);

	case NAS_MSG_CIPHER_MODE_COMMAND:
		return nas_a_make_cipher_mode_command(caller_fi, &nas_enc_msg->cipher_mode_command);

	case NAS_MSG_HANDOVER_REQUIRED_REJECT:
		return gsm0808_create_handover_required_reject(&nas_enc_msg->handover_required_reject);

	case NAS_MSG_HANDOVER_REQUEST:
		return nas_a_make_handover_request(caller_fi, &nas_enc_msg->handover_request);

	case NAS_MSG_HANDOVER_REQUEST_ACK:
		return nas_a_make_handover_request_ack(caller_fi, &nas_enc_msg->handover_request_ack);

	case NAS_MSG_HANDOVER_COMMAND:
		return nas_a_make_handover_command(caller_fi, &nas_enc_msg->handover_command);

	case NAS_MSG_HANDOVER_SUCCEEDED:
		return gsm0808_create_handover_succeeded();

	case NAS_MSG_HANDOVER_FAILURE:
		return nas_a_make_handover_failure(caller_fi, nas_enc_msg);

	default:
		LOG_NAS_A_ENC(caller_fi, LOGL_ERROR, "Unimplemented NAS-encode message type: %s\n",
			       nas_msg_type_name(nas_enc_msg->msg_type));
		return NULL;
	}
}

struct msgb *nas_a_encode(struct osmo_fsm_inst *caller_fi, const struct nas_msg *nas_enc_msg)
{
	struct msgb *msg = _nas_a_encode(caller_fi, nas_enc_msg);
