...
struct mncc *mncc_find_by_callref_from_msg(const union mncc_msg *msg)
{
	uint32_t callref;
	...
	return mncc_find_by_callref(callref);
}
...
int mncc_tx_to_cc(struct gsm_network *net, int msg_type, void *arg)
{
	...
	struct mncc *mncc = NULL;
	OSMO_ASSERT(msg_type == msg->msg_type);

	if (msg->msg_type == MNCC_SETUP_REQ) {
		/* Incoming call to forward for inter-MSC Handover? */
		mncc = msc_t_check_call_to_handover_number(&msg->signal);
		if (mncc)
			LOG_MNCC(mncc, LOGL_DEBUG, "Incoming call matches pending inter-MSC Handover Number\n");
	}
	if (!mncc) {
		...
		mncc = mncc_find_by_callref_from_msg(msg);
	}
	if (mncc) {
		mncc_rx(mncc, msg);
		return 0;
