...
static int msc_i_nas_enc(struct msc_i *msc_i, const struct nas_msg *nas_enc_msg)
{
	struct msgb *l3 = msc_role_nas_encode(msc_i->c.fi, nas_enc_msg);
	if (!l3)
		return -EIO;
...
int msc_i_ready_decode_cb(struct osmo_fsm_inst *msc_i_fi, void *data, const struct nas_msg *msg)
{
...
void msc_i_fsm_ready(struct osmo_fsm_inst *fi, uint32_t event, void *data)
{
	...
	case MSC_EV_FROM_RAN_UP_L2:
		...
		msc_role_nas_decode(msc_i->c.fi, an_apdu, msc_i_ready_decode_cb, an_apdu);
		break;
...
void msc_i_fsm_clearing_onenter(struct osmo_fsm_inst *fi, uint32_t prev_state)
{
	...
	msc_i_nas_enc(msc_i, &msg);
}

int msc_i_clearing_decode_cb(struct osmo_fsm_inst *msc_i_fi, void *data, const struct nas_msg *msg)
{
...
void msc_i_fsm_clearing(struct osmo_fsm_inst *fi, uint32_t event, void *data)
{
	...
	case MSC_EV_FROM_RAN_UP_L2:
		an_apdu = data;
		msc_role_nas_decode(msc_i->c.fi, an_apdu, msc_i_clearing_decode_cb, NULL);
		return;
...
void msc_i_fsm_cleanup(struct osmo_fsm_inst *fi, enum osmo_fsm_term_cause cause)
{
	...
	mncc_release(msc_i->inter_msc.mncc_forwarding_to_remote_cn);
