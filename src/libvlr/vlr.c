				      ...
				      struct osmo_gsup_message *gsup_msg)
{
	...
	gsup_msg->kind = OSMO_GSUP_KIND_SUBSCRIBER_MANAGEMENT;
...
{
	struct osmo_gsup_message gsup_msg = {
		.kind = OSMO_GSUP_KIND_SUBSCRIBER_MANAGEMENT,
	};
...
int vlr_start(struct vlr_instance *vlr, struct gsup_client_mux *gcm)
{
	...
	vlr->gcm = gcm;
	gcm->rx_cb[OSMO_GSUP_KIND_SUBSCRIBER_MANAGEMENT] = (struct gsup_client_mux_rx_cb){
		.func = vlr_gsup_rx,
		.data = vlr,
	};
