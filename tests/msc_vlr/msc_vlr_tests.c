...
extern int nas_dec_dtap_undup_pdisc_ctr_bin(uint8_t pdisc);
...
static void patch_l3_seq_nr(struct msgb *msg)
{
	...
	int bin = nas_dec_dtap_undup_pdisc_ctr_bin(pdisc);
...
static int bssap_validate_cipher_mode_cmd(const struct nas_cipher_mode_command *cmd)
{
	int i;
	const char *got_key;
...
static int iucs_validate_security_mode_ctrl(const struct nas_cipher_mode_command *cmd)
{
	const char *got_ik;
...
struct msgb *dont_nas_encode(struct osmo_fsm_inst *caller_fi, const struct nas_msg *nas_enc_msg)
{
	...
	LOG_NAS_ENC(caller_fi, DMSC, LOGL_INFO, "%s on %s\n", nas_msg_type_name(nas_enc_msg->msg_type),
		     ran_name);

	switch (nas_enc_msg->msg_type) {
	case NAS_MSG_DTAP:
		_validate_dtap(nas_enc_msg->dtap, ran_type);
		break;
	...
	case NAS_MSG_CIPHER_MODE_COMMAND:
		switch (ran_type) {
		case OSMO_RAT_GERAN_A:
			bssap_validate_cipher_mode_cmd(&nas_enc_msg->cipher_mode_command);
			break;
		case OSMO_RAT_UTRAN_IU:
			iucs_validate_security_mode_ctrl(&nas_enc_msg->cipher_mode_command);
			break;
		default:
			OSMO_ASSERT(false);
		}
...
static int fake_msc_a_nas_dec(const struct nas_msg *nas_dec_msg)
{
	struct msc_a_nas_dec_data d = {
		.from_role = MSC_ROLE_I,
	};
	return msc_a_nas_decode_cb(g_msub->role[MSC_ROLE_A], &d, nas_dec_msg);
}
...
void rx_from_ms(struct msgb *msg)
{
	...
	patch_l3_seq_nr(msg);
	fake_msc_a_nas_dec(&nas_dec_msg);
...
{
	struct nas_msg nas_dec = {
		.msg_type = NAS_MSG_CLASSMARK_UPDATE,
		.classmark_update = {
			.classmark = classmark,
		},
	};
	fake_msc_a_nas_dec(&nas_dec);
}
...
static int ms_sends_msg_fake(uint8_t pdisc, uint8_t msg_type)
{
	...
	nas_dec = (struct nas_msg){
		.msg_type = NAS_MSG_DTAP,
		.dtap = msg,
	};
	rc = fake_msc_a_nas_dec(&nas_dec);
...
void ms_sends_ciphering_mode_complete(const char *inner_nas_msg)
{
	...
	if (inner_nas_msg) {
		struct msgb *msg = msgb_from_hex("cipher_mode_complete_nas", 1024, inner_nas_msg);
		msg->l1h = msg->l2h = msg->l3h = msg->data;
		nas_dec = (struct nas_msg){
			.msg_type = NAS_MSG_DTAP,
			.dtap = msg,
