...
struct msc_a {
	...
	struct {
		...
		struct mncc *mncc_forwarding_to_remote_ran;
...
#define msc_a_nas_down(A,B,C) \
	_msc_a_nas_down(A,B,C, __FILE__, __LINE__)
int _msc_a_nas_down(struct msc_a *msc_a, enum msc_role to_role, const struct nas_msg *nas_enc_msg,
...
int _msc_a_msg_down(struct msc_a *msc_a, enum msc_role to_role, uint32_t to_role_event,
		    const struct nas_msg *nas_enc_msg,
		    const char *file, int line);
...
struct msgb *msc_a_nas_encode(struct msc_a *msc_a, const struct nas_msg *nas_enc_msg);
